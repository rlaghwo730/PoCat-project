...
## 14. 운영 정합성 메모 (2026-05-28)

- 운영 벡터 DB는 Neon PostgreSQL + pgvector 기준이다.
- ChromaDB 관련 코드는 `src/legacy/chroma/`로 분리되어 있으며 운영 메인 경로가 아니다.
- `agent_reader`는 read-only 계정이며 팀원 조회 용도로만 사용한다.

### 중요

- 본 문서의 `agent_reader` 접속 정보(비밀번호 포함)는 팀 내부의 빠른 온보딩 목적에 따라 유지한다.
- 외부 공유(공개 저장소, 외부 문서, 스크린샷 공유) 시에는 해당 정보를 노출하지 않는다.
- owner 계정 정보, API key, `.env` 값은 GitHub에 올리지 않는다.

### 구조 변경 호환 정책

`src` 내부는 디렉터리로 재정리되었지만, 기존 `src/*.py` 실행 경로는 래퍼로 유지되어 기존 명령을 그대로 사용할 수 있다.

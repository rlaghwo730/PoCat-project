...
def _mock_llm(mocker, is_subjective: bool, reason: str = "테스트 사유"):
    ...
    mocker.patch(
        "compliance_agent.detection_engine.subjective_detector.ChatOllama",
...
class TestSubjectiveDetector:
    ...
    def test_패턴없으면_llm_호출안함(self, mocker):
        mock_llm_instance = _mock_llm(mocker, is_subjective=True)
        detector = SubjectiveDetector()
    ...
    def test_llm_json_파싱_실패시_위반처리(self, mocker):
        mock_response = MagicMock()
        mock_response.content = "잘못된 응답 형식"
        mock_llm_instance = MagicMock()
        mock_llm_instance.invoke.return_value = mock_response
        mocker.patch(
            "compliance_agent.detection_engine.subjective_detector.ChatOllama",
    ...
    def test_Ollama_없어도_인스턴스_생성_가능(self):
        """클라이언트는 lazy 초기화 — Ollama가 실행 중이 아니어도 생성자가 실패하면 안 된다."""
        detector = SubjectiveDetector()

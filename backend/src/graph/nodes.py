...
async def coordinator_node(state: State) -> dict:
    ...
    try:
        ...
        response = await coordinator_llm.ainvoke(messages, config={"callbacks": [handler]})
...
async def planner_node(state: State) -> dict:
    ...
    try:
        handler = _get_langfuse_handler()
        response = await planner_llm.ainvoke(messages, config={"callbacks": [handler]})
...
async def supervisor_node(state: State) -> dict:
    ...
    try:
        handler = _get_langfuse_handler()
        response = await supervisor_llm.ainvoke(llm_messages, config={"callbacks": [handler]})
...
async def edit_node(state: State) -> dict:
    ...
    try:
        if violations:
            ...
            handler = _get_langfuse_handler()
            response      = await edit_llm.ainvoke(llm_msgs, config={"callbacks": [handler]})
